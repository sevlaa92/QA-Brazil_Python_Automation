...
class UrbanRoutesPage:
    ...
    switch_blanket_active = (By.CSS_SELECTOR,
                             '#root > div > div.workflow > div.workflow-subcontainer > div.tariff-picker.shown > div.form > div.reqs.open > div.reqs-body > div:nth-child(1) > div > input')
    ...
    def click_number_text(self, telefone):
        self.driver.find_element(*self.number_text_locator).click() #Click number

        self.driver.find_element(*self.number_enter).send_keys(telefone)#Tap the number

        self.driver.find_element(*self.number_confirm).click() #Confirm ur number

        code = retrieve_phone_code(self.driver)
        code_input = WebDriverWait(self.driver, timeout=4).until(
            EC.visibility_of_element_located(self.number_code)
        )
        code_input.clear()
        code_input.send_keys(code)

        self.driver.find_element(*self.code_confirm).click() #To confirm
        # Number confirmation
    def numero_confirmado(self):
            #Retorna o número de telefone exibido na tela após a confirmação
